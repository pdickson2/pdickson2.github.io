...
SET od=W:\desiredsavelocation\%pre%
...
saga_cmd grid_tools 3 -GRIDS= -NAME=%pre%Mosaic -TYPE=9 -RESAMPLING=0 -OVERLAP=1 -MATCH=0 -TARGET_OUT_GRID=%od%\%pre%mosaic.sgrd
...
saga_cmd ta_channels 0 -ELEVATION=%od%\%pre%sinkremoveddem.sgrd  -INIT_GRID=%od%\%pre%flowaccumulation.sgrd  -CHNLNTWRK=%od%\%pre%channelnetwork.sgrd -CHNLROUTE=%od%\%pre%channelroutes.sgrd -SHAPES=%od%\%pre%shapes.sgrd
